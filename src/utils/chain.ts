...
const startGoalPrompt = new PromptTemplate({
    template:
        "You are an autonomous task creation AI called AgentGPT. You have the following objective `{goal}`. " +
        "Create a list of zero to three tasks to be completed by your AI system such that your goal is more closely reached " +
        "or completely reached.\n{format_instructions}. Please answer with Chinese",
    inputVariables: ["goal"],
    partialVariables: {
...
const executeTaskPrompt = new PromptTemplate({
    template:
        "You are an autonomous task execution AI called AgentGPT. You have the following objective `{goal}`. " +
        "You have the following tasks `{task}`. Execute the task and return the response as a string. Please answer with Chinese",
...
const createTaskPrompt = new PromptTemplate({
    template:
        "You are an AI task creation agent. You have the following objective `{goal}`. You have the following incomplete tasks `{tasks}` " +
        "and have just executed the following task `{lastTask}` and received the following result `{result}`. Based on this, " +
        "create a new task to be completed by your AI system ONLY IF NEEDED such that your goal is more closely reached or " +
        "completely reached.\n{format_instructions}. Please answer with Chinese",
